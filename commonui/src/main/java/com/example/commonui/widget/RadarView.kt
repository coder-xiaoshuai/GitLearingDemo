...
import com.example.commonui.widget.util.*
...
class RadarView @JvmOverloads constructor(context: Context?, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : View(context, attrs, defStyleAttr) {
    ...
    /**
     * 动画事件，为0代表没有动画
     * NOTE: 如果是速度一定模式下，代表从雷达中心执行动画到定点的时间
     */
    ...
    fun setProgressList(processList: List<Int>) {
        this.mProgressArray = processList.toTypedArray()
        initView()
    }
     ...
     * 设置执行动画前的进度
     */
    fun setOldProcessList(oldProcessList: List<Int>) {
        this.mOldProgressArray = oldProcessList.toTypedArray()
        initView()
    }
    ...
    private fun drawWeb(canvas: Canvas) {
        ...
        mTextArray.forEachIndexed { index, _ ->
            ...
            for (i in 1..4) {
                ...
                //且顺时针旋转（rDeg / 2）度，然后绘制长度为（lineW /4f）的实线
                canvas.save()
    ...
    private fun drawConnectionArea(canvas: Canvas) {
        ...
        val rDeg = 360f / mTextArray.size
        //根据雷达图第一个坐标最为基座标进行相应计算，算出各个进度坐标
